...
Сохранение на диск: инструмент `render_vray` принимает `save_path`: путь
с расширением `.png` — файл; иначе (существующая папка, слеш на конце или путь
без расширения) — папка, создаётся, файл `stultus_vray_<дата>.png` в ней.
