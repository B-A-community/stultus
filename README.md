...
- **Сцена в запросе.** К каждому сообщению прикладывается компактный снимок
  модели: единицы, выделение, объекты верхнего уровня с габаритами в мм,
  слои, материалы, камера. Глубже модель смотрит сама через Ruby.
...
Инструменты модели: `execute_ruby`, `get_scene`, `take_screenshot`, `undo`,
...
Проверено на живом SketchUp 2024 (24.0.484) с Codex: ход «построй куб»
проходит цикл сцена → код → проверка. Claude проверяется на gateway с
подпиской.
...
docs/                     ARCHITECTURE, PROTOCOL, DESIGN-BRIEF
