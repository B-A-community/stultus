...
- **Размер постпродакшна.** Встроенный генератор Codex отдаёт ~1,6 мегапикселя
  и игнорирует размер входа и просьбы «4K» в тексте. Большие кадры собираются
  из плиток (`size: large`, `server/src/tiles.ts`), у стыков возможны артефакты.
  Настоящий 4K одной картинкой — только платный Images API, в плагине его нет.
