...
- **Массинг по карте.** «Сделай массинг по адресу …» или ссылка на
  Яндекс.Карты / 2GIS / Google / OSM (можно на панораму): контуры и
  этажность зданий берутся из OpenStreetMap (зеркало VK Карт; по ключам —
  Яндекс и 2GIS), в модели появляется группа объёмов с адресами и высотами,
  оценённые высоты помечены.
- **Панорамы улиц.** «Посмотри на этот дом, посчитай этажи»: модель
  получает кадр с панорамы Яндекса как обычную фотографию и правит по нему
  массинг. Браузер на сервере не нужен.
...
- **Размер постпродакшна.** Встроенный генератор Codex отдаёт ~1,6 мегапикселя
  и игнорирует размер входа и просьбы «4K» в тексте. Большие кадры 4K/6K/8K
  собираются из плиток (`size`, `server/src/tiles.ts`), пустые плитки берутся
  из эталона, полный файл идёт в окно кусками. Настоящий 4K одной картинкой —
  только платный Images API, в плагине его нет.
