 ...
 * приём: имя, описание, параметры, Ruby-код с плейсхолдерами. Дальше
...
export function recipesForPrompt(): string {
  ...
  return `КОПИЛКА ПРИЁМОВ БЮРО (проверенные способы, сохранённые по просьбе пользователей). Перед построением\nпроверь, нет ли подходящего; если есть — возьми код через get_recipe и подставь параметры.\n${lines.join('\n')}`
}
