/* Presentation-only additions; gateway and Ruby handlers stay in app.js. */
...
function sync(){const busy=app.dataset.state==='busy';$('input').disabled=app.dataset.state!=='connected';$('providerSelect').disabled=busy;$('modelSelect').disabled=busy;const scene=$('attachScene').checked;$('contextLabel').textContent=scene?'Контекст сцены':'Без контекста';}
